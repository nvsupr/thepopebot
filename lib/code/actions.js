 ...
 */
export async function getWorkspaceDiffStats(id) {
  const user = await requireAuth();
  const workspace = getCodeWorkspaceById(id);
  if (!workspace || workspace.userId !== user.id || !workspace.containerName) {
    return { success: false };
  ...
  try {
    ...
    const raw = await execInContainer(
      workspace.containerName,
      `cd /home/coding-agent/workspace && git diff --shortstat origin/${baseBranch} 2>/dev/null`,
    );

    if (!raw || !raw.trim()) {
      return { success: true, insertions: 0, deletions: 0 };
    }
 ...
 * @param {string} id - Workspace ID
 * @param {string} command - 'commit-to-main' | 'create-pr' | 'draft-pr' | 'rebase'
 * @returns {Promise<{success: boolean, containerName?: string, message?: string}>}
 */
export async function runWorkspaceCommand(id, command) {
  ...
  try {
    const { volumeExists, runWorkspaceCommandContainer } = await import('../tools/docker.js');

    const hasVolume = await volumeExists(id);
    if (!hasVolume) {
      return { success: false, message: 'Start coding first.' };
    }

    // Normalize draft-pr to create-pr with draft flag
    const isDraft = command === 'draft-pr';
    const resolvedCommand = isDraft ? 'create-pr' : command;

    const shortId = id.replace(/-/g, '').slice(0, 8);
    const containerName = `command-${resolvedCommand}-${shortId}`;

    // Build prompt based on command type
    const prompts = {
      'commit-to-main': 'All changes have been staged with git add -A. Review the staged diff, write a clear conventional commit message, and run git commit with that message. Do not modify any files. Do not skip any staged changes.',
      'create-pr': isDraft
        ? `Push has already been done. Review all commits on branch ${workspace.featureBranch || ''} compared to ${workspace.branch || 'main'}. Create a **draft** pull request using gh pr create --draft with a clear title and detailed description summarizing the changes.`
        : `Push has already been done. Review all commits on branch ${workspace.featureBranch || ''} compared to ${workspace.branch || 'main'}. Create a pull request using gh pr create with a clear title and detailed description summarizing the changes.`,
    };
    const prompt = prompts[resolvedCommand] || null;

    await runWorkspaceCommandContainer({
      containerName,
