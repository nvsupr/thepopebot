  ...
  try {
    ...
    if (headlessContainer) {
      try {
        ...
        if (exitCode === 0) {
          const isPlanning = headlessContainer.toolName === 'plan_popebot_updates';
          const isCodeWorkspace = headlessContainer.toolName === 'start_headless_coding_agent';
          const completionMsg = isPlanning
            ? '\n\nPlanning complete.'
            : isCodeWorkspace
              ? '\n\nTask complete.'
